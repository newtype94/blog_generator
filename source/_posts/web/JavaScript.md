...
- sort() 메서드 사용법
  ...
  `javascript var array = [52, 273, 103, 32]; array.sort( function(left, right){} );`
...
1.  window 객체
2.  location 객체 = 주소와 관련된 객체
3.  navigator 객체 = 웹 브라우저와 관련된 객체
4.  history 객체 = 기록과 관련된 객체
5.  screen 객체 = 화면 전체와 관련된 객체
6.  document 객체 DOM = Document Object Model = HTML문서와 관련된 객체
...
1. location = "http://hanbit.co.kr";
2. location.href = "http://hanbit.co.kr";
3. location.assign("http://hanbit.co.kr");
4. location.replace("http://hanbit.co.kr"); // assign과는 다르게 뒤로가기 불가
...
header.onclick = function() {};
//이벤트 속성을 메서드처럼 호출해버리면 이벤트가 강제 실행됨
header.onclick();
```
...
1.  addEventListener(eventName, handler, userCapture)
